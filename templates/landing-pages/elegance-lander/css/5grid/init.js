		...
		// IE <= 8
			if (jQuery.browser.msie)
			{
				...
				// Versions that don't support CSS3 pseudo classes
					...
					{
						jQuery(function() {
							jQuery('.main-grid-class, .main-grid-class-layout, .do-5grid').after('<div style="clear: both;"></div>');
							jQuery('.main-grid-class-layout').css('position', 'relative');
							jQuery('.main-grid-class, .do-5grid').css('position', 'relative');
							jQuery('.main-grid-class > .row > :first-child, .main-grid-class-layout > .row > :first-child, .do-5grid > .row > :first-child').css('margin-left', '0');
							jQuery('.main-grid-class > .row:first-child, .main-grid-class-layout > .row:first-child, .do-5grid > .row:first-child').css('margin-top', '0');
							jQuery('.main-grid-class > .row:last-child, .main-grid-class-layout > .row:last-child, .do-5grid > .row:last-child').css('margin-bottom', '0');
						});
					}
			}
...
/*********************/
	...
	(function() {
		...
		jQuery(function() {
			...
			jQuery('.main-grid-class-layout').addClass('5grid'); 
		});
	})();
				...
				// Body
					_body
						...
						.bind('5grid_openNav', function() {
							...
							// Disable scrolling
								_body
									.bind('touchstart.main-grid-class_nav_block', function(e) {
										e.stopPropagation();
										e.preventDefault();
									});
									
								_window
									.bind('scroll.main-grid-class_nav_block', function(e) {
										e.preventDefault();
										e.stopPropagation();
							...
							// Animate
								_wrapper.add(_titlebar)
									.animate({ left: 0 }, (fast ? 0 : speed), easing, function() {
										...
										// Re-enable scrolling
											_body.unbind('touchstart.main-grid-class_nav_block');
											_window.unbind('touchmove.main-grid-class_nav_block');
