...
function landing_page_register() {
    ...
    $labels = array('name' => _x('Landing Pages', 'post type general name', 'landing-pages'), 'singular_name' => _x('Landing Page', 'post type singular name', 'landing-pages'), 'add_new' => _x('Add New', 'Landing Page', 'landing-pages'), 'add_new_item' => __('Add New Landing Page', 'landing-pages'), 'edit_item' => __('Edit Landing Page', 'landing-pages'), 'new_item' => __('New Landing Page', 'landing-pages'), 'view_item' => __('View Landing Page', 'landing-pages'), 'search_items' => __('Search Landing Page', 'landing-pages'), 'not_found' => __('Nothing found', 'landing-pages'), 'not_found_in_trash' => __('Nothing found in Trash', 'landing-pages'), 'parent_item_colon' => '');

    $args = array('labels' => $labels, 'public' => true, 'publicly_queryable' => true, 'show_ui' => true, 'query_var' => true, 'menu_icon' => LANDINGPAGES_URLPATH . '/images/plus.gif', 'rewrite' => array("slug" => "$slug", 'with_front' => false), 'capability_type' => 'post', 'hierarchical' => false, 'menu_position' => 32, 'supports' => array('title', 'custom-fields', 'editor', 'thumbnail', 'excerpt'));
...
function landing_page_category_registerTaxonomy() {
    $args = array('hierarchical' => true, 'label' => __("Categories", 'landing-pages'), 'singular_label' => __("Landing Page Category", 'landing-pages'), 'show_ui' => true, 'query_var' => true, "rewrite" => true);
...
function lp_add_fullscreen_param($link) {
    if (isset($_GET['page'])) return $link;
...
/**
 * Alter htaccess to intercept landing page traffic and display the correct variation
 */
...
if (is_admin()) {
    ...
    function lp_columns($columns) {
        $columns = array("cb" => "<input type=\"checkbox\" />", //"ID" => "ID",
            "thumbnail-lander" => __("Preview", 'landing-pages'), "title" => __("Landing Page Title", 'landing-pages'), "stats" => __("Variation Testing Stats", 'landing-pages'), "impressions" => __("Total<br>Visits", 'landing-pages'), "actions" => __("Total<br>Conversions", 'landing-pages'), "cr" => __("Total<br>Conversion Rate", 'landing-pages')
        ...
        if ($typenow === "landing-page") {
            ...
            if (in_array($typenow, $post_types)) {
                ...
                foreach ($filters as $tax_slug) {
                    ...
                    wp_dropdown_categories(array('show_option_all' => __('Show All ' . $tax_obj->label), 'taxonomy' => $tax_slug, 'name' => $tax_obj->name, 'orderby' => 'name', 'selected' => $current, 'hierarchical' => $tax_obj->hierarchical, 'show_count' => false, 'hide_empty' => true));
                }
            }
        }
    }
    ...
    function lp_sortable_columns() {
        return array('title' => 'title', 'impressions' => 'impressions', 'actions' => 'actions', 'cr' => 'cr');
    }
    ...
    function lp_trash_lander($post_id) {
        ...
        if (!isset($post) || isset($_POST['split_test'])) return;
        ...
        if ($post->post_type == 'landing-page') {
            ...
            $args = array('post_type' => 'landing-page-group', 'post_satus' => 'publish');
            ...
            if ($my_query->have_posts()) {
                ...
                while ($my_query->have_posts()) : $my_query->the_post();
                    ...
                    if (in_array($lp_id, $lp_ids)) {
                        ...
                        //print_r($this_data);
                        $new_post = array('ID' => $group_id, 'post_title' => get_the_title(), 'post_content' => $this_data, 'post_status' => 'publish', 'post_date' => date('Y-m-d H:i:s'), 'post_author' => 1, 'post_type' => 'landing-page-group');
                        //print_r($new_post);
