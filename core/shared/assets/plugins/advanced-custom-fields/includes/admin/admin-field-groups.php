	...
	function check_sync() {
		...
		if( $ids = acf_maybe_get_GET('acfsynccomplete') ) {
			...
			if( $total == 1 ) {
				...
				acf_add_admin_notice( sprintf(__('Field group synchronized. %s', 'acf'), '<a href="' . get_edit_post_link($ids[0]) . '">' . get_the_title($ids[0]) . '</a>') );

			} else {

				acf_add_admin_notice( sprintf(_n( '%s field group synchronized.', '%s field groups synchronized.', $total, 'acf' ), $total) );
	...
	<tr <?php if($i%2 == 0): ?>class="alternate"<?php endif; ?>>
		...
		<td class="post-title page-title column-title">
			...
			<div class="row-actions">
				<span class="import"><a title="<?php echo esc_attr( __('Synchronize field group', 'acf') ); ?>" href="<?php echo esc_url($url); ?>"><?php _e( 'Sync', 'acf' ); ?></a></span>
			</div>
		</td>
