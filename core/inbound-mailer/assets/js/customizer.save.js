jQuery(document).ready(function($) {
	...
	// Resize Functions
	$("#wp_content_resize, #inbound-mailer-conversion-area_ifr").height(210);
	   setTimeout(function() {
	jQuery("#inbound-mailer-conversion-area_ifr, #inbound-mailer-conversion-area_tbl, #wp_content_ifr, .mceLayout, .mceIframeContainer iframe").height(150);
	...
	jQuery('#inbound-mailer-frontend-options-container input, #inbound-mailer-frontend-options-container textarea').on("keyup", function (e) {
		var this_id = jQuery(this).attr("id");
		var parent_el = jQuery(this).parent();
		jQuery(parent_el).find(".inbound-mailer-success-message").remove();
		jQuery(parent_el).find(".new-save-inbound-mailer-frontend").remove();
		var ajax_save_button = jQuery('<span class="button-primary new-save-inbound-mailer-frontend" id="' + this_id + '" style="margin-left:10px">Update</span>');
		//console.log(parent_el);
		jQuery(ajax_save_button).appendTo(parent_el);
	});

	// On change have save button show
	jQuery('#inbound-mailer-frontend-options-container input, #inbound-mailer-frontend-options-container select, #inbound-mailer-frontend-options-container textarea').on("change", function (e) {
		var this_id = jQuery(this).attr("id");
		var parent_el = jQuery(this).parent();
		jQuery(parent_el).find(".inbound-mailer-success-message").remove();
		jQuery(parent_el).find(".new-save-inbound-mailer-frontend").remove();
	...
	setTimeout(function() {
	jQuery('.inbound-email-option-row iframe').contents().find('body').on("keyup", function (e) {
		var thisclass = jQuery(this).attr("class");
		var this_class_dirty = thisclass.replace("mceContentBody ", "");
		var this_class_cleaner = this_class_dirty.replace("wp-editor", "");
		var clean_1 = this_class_cleaner.replace("post-type-inbound-email", "");
		...
		console.log(this_id);
		var parent_el = jQuery( "." + this_id + " .inbound-email-table-headerz");
		jQuery(parent_el).find(".inbound-mailer-success-message").remove();
		jQuery(parent_el).find(".new-save-inbound-mailer-frontend").remove();
		var ajax_save_button = jQuery('<span class="button-primary new-save-inbound-mailer-frontend" id="' + this_id + '" style="margin-left:10px;">Update</span>');
		//console.log(parent_el);
	...
	// Prep Data and Save
	var nonce_val = inbound_email_post_edit_ui.wp_call_to_action_meta_nonce; // NEED CORRECT NONCE
	jQuery(document).on('mousedown', '.new-save-inbound-mailer-frontend', function () {
		//alert('here');
		var type_input = jQuery(this).parent().find("input").attr("type");
		var type_select = jQuery(this).parent().find("select");
		// var the_conversion_area_editor = jQuery(this).parent().parent().find('#inbound-mailer-conversion-area_ifr').length;
		jQuery(this).parent().find(".inbound-mailer-success-message").hide();
	   ...
	   // console.log(new_value_meta_select);
		...
		jQuery.ajax({
			type: 'POST',
			url: inbound_email_post_edit_ui.ajaxurl,
			context: this,
			data: {
			...
			success: function (data) {
				...
				//alert(data);
				// jQuery('.inbound-mailer-form').unbind('submit').submit();
				//var worked = '<span class="success-message-map">Success! ' + this_meta_id + ' set to ' + meta_to_save + '</span>';
				var worked = '<span class="inbound-mailer-success-message">Updated!</span>';
				var s_message = jQuery(self).parent();
				jQuery(worked).appendTo(s_message);
				jQuery(self).parent().find("inbound-mailer-success-message").remove();
				jQuery(self).hide();
				jQuery('.reload').click();
				//alert("Changes Saved!");
			},
	...
	jQuery('.full-size-view').on('click', function (event) {
		jQuery(parent.document).find("#inbound-mailer-live-preview").contents().find('html').removeClass("small-html");
		jQuery(parent.document).find("#inbound-mailer-live-preview").contents().find('html').css("width", "100%");
		jQuery(this).hide();
		jQuery('.shrink-view').show();
		$.cookie("inbound-mailer-view-choice", "full-size", { path: '/', expires: 7 });
	});

	jQuery('.shrink-view').on('click', function (event) {
		jQuery(parent.document).find("#inbound-mailer-live-preview").contents().find('html').css("width", "125%");
		jQuery(parent.document).find("#inbound-mailer-live-preview").contents().find('html').addClass("small-html");
		jQuery(this).hide();

		jQuery('.full-size-view').show();
		$.cookie("inbound-mailer-view-choice", "shrink", { path: '/', expires: 7 });
	});
	...
	function reload_preview() {
		...
		var reload_url = parent.window.location.href;
		reload_url = reload_url.replace('email-customizer=on','');
		//alert(reload_url);
		var current_variation_id = jQuery("#current_variation_id").text();

		// var reload = jQuery(parent.document).find("#inbound-mailer-live-preview").attr("src");
		var new_reload = reload_url + "&live-preview-area=" + cache_bust + "&inbvid=" + current_variation_id;

		jQuery(parent.document).find("#inbound-mailer-live-preview").attr("src", new_reload);
	...
	jQuery('.inbound-email-option-row').on('mouseover', function (event) {
		var $tgt = jQuery(event.target);
		var domElement = jQuery(event.target);
		var current_el_id = jQuery(this).find('input[type="text"], textarea, select').attr("id");
		var finding_the_match = jQuery(parent.document).find("#inbound-mailer-live-preview").contents().find("#" + current_el_id);
		...
		if ( match_color === "rgb(255, 255, 255)") {
			jQuery(finding_the_match).addClass('inbound-mailer-see-this');
		}
	...
	jQuery('.inbound-email-option-row').on('mouseout', function (event) {
		var $tgt = jQuery(event.target);
		 var domElement = jQuery(event.target);
		var current_el_id = jQuery(this).find('input[type="text"], textarea, select').attr("id");
		var finding_the_match = jQuery(parent.document).find("#inbound-mailer-live-preview").contents().find("#" + current_el_id);
		...
		if ( match_color === "rgb(0, 0, 0)") {
			jQuery(finding_the_match).removeClass('inbound-mailer-see-this');
		}
	...
	jQuery('input[type="text"], textarea, select').each(function(){
		...
		jQuery(this).on("keyup", function (e) {
			...
			jQuery(parent.document).find("#inbound-mailer-live-preview").contents().find("#" + current_id).html(new_value_show);
		});
	...
	*/
	...
	    jQuery('#wp_content_ifr').contents().find("body").on({
			keyup: function() {
				...
				jQuery(parent.document).find("#inbound-mailer-live-preview").contents().find("#the-content").html(new_value);
				parent_el = jQuery(".the-content-label");
				jQuery(parent_el).find(".inbound-mailer-success-message").remove();
				jQuery(parent_el).find(".new-save-inbound-mailer-frontend").remove();
				var ajax_save_button = jQuery('<span class="button-primary new-save-inbound-mailer-frontend" id="the_content" style="margin-left:10px;">Update</span>');
				jQuery(ajax_save_button).appendTo(parent_el);
			},
...
/* might still need
	...
	setTimeout(function() {
	      jQuery('#inbound-mailer-conversion-area_ifr').contents().find("body").on({
				keyup: function() {
					//console.log('new vale');
					var new_value = jQuery(this).html();
					jQuery(parent.document).find("#inbound-mailer-live-preview").contents().find("#inbound-mailer-conversion-area").html(new_value);
					jQuery(parent.document).find("#inbound-mailer-live-preview").contents().find("#inbound_email_container_form").html(new_value);
					parent_el = jQuery(".inbound-mailer-conversion-area-label");
					jQuery(parent_el).find(".inbound-mailer-success-message").remove();
					jQuery(parent_el).find(".new-save-inbound-mailer-frontend").remove();
					var ajax_save_button = jQuery('<span class="button-primary new-save-inbound-mailer-frontend" id="inbound-mailer-conversion-area" style="margin-left:10px;">Update</span>');
					//console.log(parent_el);
...
*/
	...
	//jQuery(parent.document).find('#inbound_email_customizer_options').contents().find('#wp_content_ifr').contents().find("body").html();
	...
	jQuery('#inbound-mailer-frontend-options-container .upload_image_button').on('click', function (event) {
