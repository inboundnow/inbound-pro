...
jQuery(document).ready(function($) {
	...
	var shortcode_copy = jQuery("#inbound_email_shortcode_form");
	...
	jQuery("input.cta-width").parent().find(".inbound_email_tooltip").after(calc);
	jQuery(".calc.button-secondary").css('display', 'inline-block');

	// Fix Split testing iframe size
	jQuery("#inbound-mailer-metabox-splittesting a.thickbox, #leads-table-container-inside .column-details a").live('click', function () {
	...
	var current_template = jQuery("input#inbound_email_select_template ").val();
	var current_template_meta = "#inbound_email_" + current_template + "_custom_meta_box";
	jQuery(current_template_meta).removeClass("postbox").appendTo("#template-display-options").addClass("Old-Template");
	var current_template_h3 = "#inbound_email_" + current_template + "_custom_meta_box h3";
	...
	jQuery('.inbound_email_select_template').click(function(){
		...
		var current_template_meta = "#inbound_email_" + current_template + "_custom_meta_box";
		var current_template_h3 = "#inbound_email_" + current_template + "_custom_meta_box h3";
		var current_template_div = "#inbound_email_" + current_template + "_custom_meta_box .handlediv";
		...
		jQuery(".inbound-mailer-template-selector-container").fadeOut(500,function(){
			...
			var ajax_data = {
				action: 'inbound_email_get_template_meta',
				selected_template: template,
				post_id: inbound_email_post_edit_ui.post_id,
				post: inbound_email_post_edit_ui.post_id,
			};
		...
		jQuery('#inbound_email_metabox_select_template h3').first().html('Current Active Template: '+label);
	...
	jQuery('.jpicker').one('mouseenter', function () {
		...
		// alert('Color chosen - hex: ' + (all && '#' + all.hex || 'none') + ' - alpha: ' + (all && all.a + '%' || 'none'));
			...
			jQuery(this).parent().find(".inbound-mailer-success-message").remove();
			jQuery(this).parent().find(".new-save-wp-cta").show();
			jQuery(this).parent().find(".new-save-inbound-mailer-frontend").show();

			//jQuery(this).attr('value', all.hex);
		});
	});

	if (jQuery(".inbound-mailer-template-selector-container").css("display") == "none"){
	...
	var selected_template = jQuery('#inbound_email_select_template').val();
	...
	*/
	...
	jQuery('.current_lander .background-style').live('change', function () {
		...
		if (input == 'color') {
			...
			var obj = jQuery(".current_lander tr.background-style td .inbound_email_tooltip");
			obj.removeClass("inbound_email_tooltip").addClass("background_tip").html("Use the custom css block at the bottom of this page to set up custom CSS rules");
	...
	if (post_status === "draft") {
		// jQuery( ".nav-tab-wrapper.a_b_tabs .inbound-mailer-ab-tab, #tabs-add-variation").hide();
		jQuery(".new-save-inbound-mailer-frontend").on("click", function(event) {
			event.preventDefault();
			alert("Must publish this page before you can use the visual editor!");
		});
		var subbox = jQuery("#submitdiv");
		jQuery("#inbound_email_ab_display_stats_metabox").before(subbox);
...
function getURLParameter(name) {
	...
	jQuery('#main-title-area input').on("change keyup", function (e) {
		...
		var current_view = jQuery("#inbound-mailer-current-view").text();
		if (current_view !== "0") {
	...
	jQuery("body").on('click', '.new-save-wp-cta', function () {
		...
		//alert(reload_url);
		var current_variation_id = jQuery("#inbound-mailer-current-view").text();
		...
		jQuery(parent.document).find("#inbound-mailer-live-preview").css('width', iframe_w).css('height', iframe_h);
		}
