...
class Campaign extends CCObject{
    ...
    public function __construct($params=array()){
		...
		$this->orgCity = (isset($params['orgCity'])) ? $params['orgCity'] : '$ACCOUNT.CITY';
		$this->orgState = (isset($params['orgState'])) ? $params['orgState'] : '$ACCOUNT.STATE';
		$this->orgInternationalState = (isset($params['orgInternationalState'])) ? $params['orgInternationalState'] : '';
		$this->orgCountry = (isset($params['orgCountry'])) ? $params['orgCountry'] : '$ACCOUNT.COUNTRY';
		$this->orgPostalCode = (isset($params['orgPostalCode'])) ? $params['orgPostalCode'] : '$ACCOUNT.POSTALCODE';
    ...
    public function createXml(){
        ...
        $xml = $xml->asXML();
//mail('kostroma24@yandex.ru', 'f515', 'sss '. $xml);

		//Convert utf8 to latin1. All characters above 255 convert to char references
		$convmap= array(0x0100, 0xFFFF, 0, 0xFFFF);
		$encutf= mb_encode_numericentity($xml, $convmap, 'UTF-8');
		$xml= utf8_decode($encutf);
...
class Schedule extends CCObject{
    ...
    public function createXml(){
        ...
        $xml->addChild("id", $this->campaign->id.'/schedules/1');
        $title = $xml->addChild("title", $this->time);
		$title->addAttribute("type", "text");
        ...
        $schedule->addAttribute("id", $this->campaign->id.'/schedules/1');
