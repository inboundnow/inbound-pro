<?php
/*
Plugin Name: Inbound Extension - Zapier Integration
...
if (!class_exists('Inbound_Zapier')) {
	...
	class Inbound_Zapier {
		...
		static function inboundnow_zapier_extension_setup() {
			...
			if ( class_exists( 'Inbound_License' ) )
				$license = new Inbound_License( INBOUNDNOW_ZAPIER_FILE , INBOUNDNOW_ZAPIER_LABEL , INBOUNDNOW_ZAPIER_SLUG , INBOUNDNOW_ZAPIER_CURRENT_VERSION  , INBOUNDNOW_ZAPIER_REMOTE_ITEM_NAME ) ;
		...
		static function inboundnow_zapier_add_subscriber( $lead_data , $webhook_url ) {
			...
			self::inboundnow_zapier_send_data( $lead_data_encoded , $webhook_url );
		}


		public static function inbound_zap_generate_lead() {
			// Sample Lead Data
			$lead_data = array(
				"user_ID" => 1,
			...
			{
				self::inboundnow_zapier_add_subscriber( $lead_data, $webhook_url );
			}
		}
									...
									console.log("clcickckckc");
												jQuery.ajax({
													...
													success: function (e) {
														...
														jQuery(".copy-right-area").html(r.replace);
													}
												})
											});
								 });
		...
		static function inboundnow_zapier_inboundnow_form_integratation($form_post_data , $form_meta_data ) {
			...
			if (isset($form_settings['inbound_shortcode_zapier_enable']) && $form_settings['inbound_shortcode_zapier_enable']=='on') {
				...
				foreach ($webhook_urls as $webhook_url) {
					...
					self::inboundnow_zapier_add_subscriber( $form_post_data , $webhook_url );
				...
				switch($action) {
					case 'export-zapier':
						...
						foreach( $post_ids as $post_id ) {
							...
							self::inboundnow_zapier_add_subscriber( $lead_data );
	...
	add_action( 'plugins_loaded' , 'inbound_zapier_register_welcome_class' , 20 ); 
	function inbound_zapier_register_welcome_class() {
