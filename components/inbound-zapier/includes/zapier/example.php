...
//Exercise Blog API
    ...
    echo $blogs->create_post('0d61e4ca-e395-4c1c-8766-afaa48bf68db', 'Test Author', 'testapi@zapier.com', 'find this one', 'testing hapihp summary', 'this is the content for testing hapihp', $tags);
    ...
    echo $blogs->create_comment('ae15391a-7bd9-454d-b51b-5c480a010497', 'hapihp comment Author', 'hapihp@hs.com', 'http://zapier.com', 'hapihp test comment created a new one now');


//Exercise Leads API
    $leads = new Zapier_Leads($HAPIKey);
    ...
    //Add a lead
    $postValues = array('email'=>'haPiHPtest@zapier.com',
                        'firstName'=>'Posted',
                        'lastName'=>'By',
                        'company'=>'haPiHP');
    echo $leads->add_lead('http://demohubapi.app6.zapier.com/?app=leaddirector&FormName=testform', $postValues);
...
//Exercise Settings API
    $settings = new Zapier_Settings($HAPIKey);
...
//Exercise Marketing Events API
    $events = new Zapier_Events($HAPIKey);
...
//Exercise Lead Nurturing API
    $nurture = new Zapier_LeadNurturing($HAPIKey);
...
//Exercise Prospects API
    $prospects = new Zapier_Prospects($HAPIKey);
...
//Exercise Keywords API
    $keywords = new Zapier_Keywords($HAPIKey);
...
/*    $contacts = new Zapier_Contacts($HAPIKey);
    ...
    print_r($deletedContact);
        ...
        print_r($contactByID);
        $contactByEmail = $contacts->get_contact_by_email('wgordon+test@zapier.com');
    ...
    //Exercise Workflows API
        $workflows = new Zapier_Workflows($HAPIKey);
    ...
    //Exercise Forms API
        $forms = new Zapier_Forms($HAPIKey);
        $uid = uniqid();
    ...
    //Exercise Lists API
        $lists = new Zapier_Lists($HAPIKey);
        ...
        $updated_list_array = array('name'=>'Tweeters and Zapierters','dynamic'=>false,'portalId'=>'62515','filters'=>
            array(array(array('operator'=>'IS_NOT_EMPTY','property'=>'twitterhandle','type'=>'string'),
                array('operator'=>'EQ','value'=>'Zapier','property'=>'company','type'=>'string'))));
    ...
    //Exercise Properties API
        $properties = new Zapier_Properties($HAPIKey);
...
*/
    //Exercise Social Media API
        $social = new Zapier_SocialMedia($HAPIKey);
