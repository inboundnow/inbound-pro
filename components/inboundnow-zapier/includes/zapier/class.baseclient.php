...
* License.
*/
class Zapier_BaseClient
{
    // Zapier_BaseClient class to be extended by specific hapi clients
     ...
     * Constructor.
     *
     * @param $HAPIKey: String value of Zapier API Key for requests
     */
    public function __construct($HAPIKey,$PortalID, $userAgent="haPiHP default UserAgent")
    {
     ...
     * clients that extend Zapier_BaseClient should set $API_PATH to the
    ...
    protected function get_api()
    {
        if ( $this->isBlank($this->API_PATH) )
            throw new Zapier_Exception('API_PATH must be defined');
        else
     ...
     * clients that extend Zapier_BaseClient should set $API_VERSION to the
    ...
    protected function get_api_version()
    {
        if ( $this->isBlank($this->API_VERSION) )
            throw new Zapier_Exception('API_VERSION must be defined');
        else
    ...
    protected function execute_get_request($url)
    {
        ...
        if ( $errno > 0 )
            throw new Zapier_Exception('cURL error: ' . $error);
        else
            return $output;
    }
    ...
    protected function execute_put_request($url, $body)
    {
        ...
        if ( $apierr > 0 )
            throw new Zapier_Exception('cURL error: ' . $errmsg);
        else
            return $result;
    }
