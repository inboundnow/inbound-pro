...
Version: 1.0.2
Author: Hudson Atwell, Davdid Wells, (with special credit to Joakim Ling & Jon Bishop)
Author URI: http://www.inboundnow.com/landing-pages/
*/


	define('LP_CLEAN_URLS_VERSION','1.0.2');
	
	/*SETUP NAVIGATION AND DISPLAY ELEMENTS*/
	add_filter('lp_define_global_settings','lp_clean_urls_add_global_settings');
	function lp_clean_urls_add_global_settings($lp_global_settings)
	{
	...
	add_action('init', array('LP_REMOVESLUG', 'init'), 99);
	register_activation_hook( INBOUND_NOW_FILE , array('LP_REMOVESLUG', 'flush_rewrite_rules') );
	register_deactivation_hook( INBOUND_NOW_FILE , array('LP_REMOVESLUG', 'flush_rewrite_rules') );
